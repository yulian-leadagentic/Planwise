...
export function MessagingDashboardPage() {
  ...
  const kpiCards = [
    ...
    { label: 'Urgent Items', value: stats.urgentMessages, icon: AlertTriangle, color: stats.urgentMessages > 0 ? 'text-red-600 bg-red-100' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800' },
  ];
    ...
    <div className="space-y-6">
        ...
        actions={
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-[13px] font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50"
            >
      ...
      <div className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          <h3 className="text-[15px] font-bold text-slate-900 dark:text-slate-100">Recent Activity</h3>
        </div>
        <div className="divide-y divide-slate-50 dark:divide-slate-800">
          {(stats.recentActivity ?? []).length === 0 ? (
            <div className="py-8 text-center text-sm text-slate-400 dark:text-slate-500">No recent messages</div>
          ) : (
            (stats.recentActivity as any[]).map((msg: any) => (
              <div key={msg.id} className="flex items-center gap-3 px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] text-slate-700 dark:text-slate-200 truncate">{msg.content}</p>
