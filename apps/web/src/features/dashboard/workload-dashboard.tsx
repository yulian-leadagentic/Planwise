...
function getUtilColor(pct: number): string {
  if (pct === 0) return 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 text-center">
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400">Avg Utilization</p>
        </div>
        <div className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 text-center">
          <p className={cn('text-2xl font-bold', overloaded > 0 ? 'text-red-600' : 'text-slate-400 dark:text-slate-500')}>{overloaded}</p>
          <p className="text-[11px] text-slate-500 dark:text-slate-400">Overloaded (&gt;100%)</p>
        </div>
        <div className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 text-center">
          <p className={cn('text-2xl font-bold', underutil > 0 ? 'text-amber-600' : 'text-slate-400 dark:text-slate-500')}>{underutil}</p>
          <p className="text-[11px] text-slate-500 dark:text-slate-400">Under-utilized (&lt;50%)</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{format(weekStart, 'MMM d')} — {format(addDays(weekStart, 4), 'MMM d, yyyy')}</span>
          <button onClick={() => setWeekOffset((o) => o + 1)} className="rounded-md p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800"><ChevronRight className="h-5 w-5" /></button>
        </div>
        <div className="flex items-center gap-1 rounded-lg bg-slate-100 dark:bg-slate-800 p-0.5">
          <button onClick={() => setViewMode('utilization')} className={cn('px-3 py-1 rounded-md text-[12px] font-semibold', viewMode === 'utilization' ? 'bg-white dark:bg-slate-900 shadow-sm text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400')}>Utilization %</button>
          <button onClick={() => setViewMode('hours')} className={cn('px-3 py-1 rounded-md text-[12px] font-semibold', viewMode === 'hours' ? 'bg-white dark:bg-slate-900 shadow-sm text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400')}>Hours</button>
        </div>
      </div>
      ...
      <div className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
        <table className="w-full text-[12px]">
          ...
          <tbody>
            {members.length === 0 ? (
              <tr><td colSpan={10} className="py-8 text-center text-slate-400 dark:text-slate-500">No workload data</td></tr>
            ) : (
              members.map((m: any) => {
                const user = m.user;
                const wl = m.workload;
                ...
                return (
                  <tr key={user?.id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50/50">
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-slate-700 dark:text-slate-200 truncate">{user?.firstName} {user?.lastName}</p>
                    ...
                    {days.map((d) => {
                      ...
                      return (
                        <td key={dateStr} className="px-2 py-2.5 text-center">
                          {capacity === 0 ? (
                            <span className="text-[10px] text-slate-300 dark:text-slate-600">Off</span>
                            ...
                            <div className="text-[11px]">
                              <span className="font-medium text-slate-700 dark:text-slate-200">{planned.toFixed(1)}h</span>
                              <span className="text-slate-400 dark:text-slate-500">/{capacity.toFixed(0)}h</span>
                            </div>
                          )}
                        </td>
                      );
                    })}
                    <td className="px-3 py-2.5 text-center font-medium text-slate-700 dark:text-slate-200">{(summary.totalPlanned ?? 0).toFixed(1)}h</td>
                    <td className="px-3 py-2.5 text-center text-slate-500 dark:text-slate-400">{(summary.totalCapacity ?? 0).toFixed(0)}h</td>
                    <td className="px-3 py-2.5 text-center text-slate-500 dark:text-slate-400">{(summary.totalActual ?? 0).toFixed(1)}h</td>
                    <td className="px-3 py-2.5 text-center">
                      <div className="flex items-center gap-2 justify-center">
                        ...
                        <span className={cn('text-[11px] font-bold', (summary.avgUtilization ?? 0) > 100 ? 'text-red-600' : 'text-slate-700 dark:text-slate-200')}>
