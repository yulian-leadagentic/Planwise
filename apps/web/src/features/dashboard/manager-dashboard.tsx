...
const feasibilityColors = {
  ...
  UNKNOWN: { bg: 'bg-slate-100 dark:bg-slate-800', text: 'text-slate-500 dark:text-slate-400', label: 'Unknown' },
};
...
export function ManagerDashboard() {
  ...
  const kpiCards = [
    ...
    { label: 'Overdue', value: kpis.overdueTasks ?? 0, icon: Clock, color: kpis.overdueTasks > 0 ? 'text-red-600 bg-red-100' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800' },
    { label: 'Blocked', value: kpis.blockedTasks ?? 0, icon: XCircle, color: kpis.blockedTasks > 0 ? 'text-amber-600 bg-amber-100' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800' },
    { label: 'At Risk', value: kpis.atRiskProjects ?? 0, icon: AlertTriangle, color: kpis.atRiskProjects > 0 ? 'text-amber-600 bg-amber-100' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800' },
    { label: 'Impossible', value: kpis.impossibleProjects ?? 0, icon: XCircle, color: kpis.impossibleProjects > 0 ? 'text-red-600 bg-red-100' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {kpiCards.map((card) => {
          ...
          return (
            <div key={card.label} className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
              ...
              <p className="text-[11px] font-medium text-slate-400 dark:text-slate-500 mt-0.5">{card.label}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-[15px] font-bold text-slate-900 dark:text-slate-100">Project Health</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {(dashboard.projects as any[]).map((p: any) => {
              ...
              return (
                <div
                  key={p.project.id}
                  className="rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 hover:border-blue-300 cursor-pointer transition-colors"
                  onClick={() => navWithReturn(`/projects/${p.project.id}`, 'Manager Dashboard')}
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{p.project.name}</h4>
                  ...
                  <div className="mb-2">
                    <div className="flex items-center justify-between text-[11px] mb-1">
                      ...
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{p.progress ?? 0}%</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 text-[10px]">
                    {Object.entries(p.statusCounts ?? {}).map(([status, count]) => (
                      <span key={status} className="rounded-full bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 text-slate-500 dark:text-slate-400">
