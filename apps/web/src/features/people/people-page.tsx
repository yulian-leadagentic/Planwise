...
/**
 * Seniority History — date-effective list of an employee's seniority
 * assignments. Replaces the single seniorityLevelId dropdown so the
 * project cost calculation can use the level that was active when each
 * TimeEntry was logged (not the current level).
 *
 * The legacy `users.seniority_level_id` column is auto-synced by the
 * service after every CRUD operation here (= the currently-open row,
 * or the most recent closed row if none open). So existing reads
 * (employee table column, dashboards, etc.) keep working unchanged.
 *
 * UI states:
 *   - Empty (no rows yet)        → "No seniority history. Add the
 *                                  first entry to start tracking cost."
 *   - One open row               → typical "current" state; admins
 *                                  click "Add new" to log a promotion
 *   - Multiple rows              → chronological list, current row
 *                                  pinned at top with "Current" pill
 *
 * Add flow:
 *   - Open-ended add (no end date) closes the previous open row at
 *     startDate - 1 automatically (server-side). UI says so inline.
 *   - Closed historical add lets admins backfill a missed period.
 */
interface SeniorityHistoryRow {
  id: number;
...
function SeniorityHistorySection({
  userId,
  seniorityLevels,
}: {
  userId: number;
