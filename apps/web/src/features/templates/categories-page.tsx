...
export function CategoriesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    staleTime: 5 * 60 * 1000,
    queryFn: () => client.get('/admin/config/service-categories').then((r) => r.data.data),
  });

  const createMutation = useMutation({
    mutationFn: (data: { name: string }) =>
      client.post('/admin/config/service-categories', data).then((r) => r.data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => client.delete(`/admin/config/service-categories/${id}`).then((r) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['service-categories'] });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <PageHeader
          ...
          description="Categories like BIM, MEP, Structural, Architecture"
          actions={
      ...
      {isLoading ? (
        ...
        <p className="py-8 text-center text-sm text-muted-foreground">No service categories yet. Click "Add Category" to create one.</p>
      ) : (
        <div className="rounded-lg border border-border">
          {categories.map((c: any, i: number) => (
            <div key={c.id} className={`flex items-center justify-between px-4 py-3 ${i < categories.length - 1 ? 'border-b border-border' : ''} hover:bg-muted/30`}>
