  ...
  return (
    <div className="space-y-6">
      ...
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> Back to deliverable templates
      </button>
...
export function DeliverableTemplatesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['templates', 'task_list'] });
      notify.success('Deliverable template created', { code: 'TPL-CREATE-200' });
      setShowForm(false);
      setName('');
      setCode('');
      setDescription('');
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['templates', 'task_list'] });
      notify.success('Deliverable template deleted', { code: 'TPL-DELETE-200' });
    },
    ...
    <div className="space-y-6">
        ...
        <PageHeader
          ...
          actions={
            <button onClick={() => setShowForm(!showForm)} className={btnPrimary}>
              <Plus className="h-4 w-4" /> New Deliverable Template
            </button>
          }
        />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-border bg-background p-8 text-center">
          ...
          <h3 className="mt-3 text-sm font-medium">No deliverable templates</h3>
