          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase tracking-wider text-slate-500">
                <th className="sticky left-0 z-10 bg-slate-50 px-3 py-3 text-left font-semibold w-16">Year</th>
                <th className="sticky left-16 z-10 bg-slate-50 px-3 py-3 text-left font-semibold min-w-[200px]">Project</th>
            ...
            <tbody>
              {data.projects.map((p, idx) => (
                ...
                >
                  <td className={cn(
                    'sticky left-0 z-10 px-3 py-2 text-slate-500 tabular-nums',
                    idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30',
                  )}>
                    {p.year ?? '—'}
                  </td>
                  <td className={cn(
                    ...
                    <Link
                      to={`/projects/${p.id}`}
                      className="block font-medium text-slate-800 hover:text-blue-600 truncate"
                      title={p.name}
                    >
                      {p.name}
                    </Link>
                    {p.number && (
                      <span className="text-[10px] font-mono text-slate-400">{p.number}</span>
                    )}
                  </td>
                  {data.milestones.map((m) => {
