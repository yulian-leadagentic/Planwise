...
export class BusinessPartnersController {
  ...
  // The old BM2 Phase E BP-admin Excel wizard (its three POST
  // endpoints under this controller — parse / preview / commit) was
  // retired in favour of the /admin/data-import contacts flow, which
  // now covers every capability the old wizard did (typed org BPs,
  // person BPs, worker_of edges, dedup) plus multi-format triage,
  // mapping presets, split & fill, idempotency, history, and project
  // attach. Both entry points (BP admin + Data Import) route to the
  // same wizard now.
}
